...
public class SqlApiTest extends BaseTest {
    ...
    @Story("Статус операции в БД")
    void shouldFailedStatusIfPaymentByDeclinedCard() {
    ...
    void shouldFieldStatusIfPaymentByCreditDeclinedCard() {
        ...
        Assertions.assertEquals("DECLINED", paymentCreditData.getStatus());
    }
    ...
    void ShouldNotAmountRowsWhenPaymentFailedCard() {
        var cardInfo = DataHelper.getFailedCardInfo();
        ...
        APIHelper.cardPaymentWhenUsedFailedCard(cardInfo);
    ...
    @Feature("Оплата картой отсутствующей в системе")
    @Story("Отсутствие новых данных в столицах ")
    void ShouldNotAmountRowsWhenPaymentByCreditFailedCard(){
        var cardInfo = DataHelper.getFailedCardInfo();
        var startRowsAmountInCreditRequestEntity = SQLHelper.getRowAmountFromCreditRequestEntityTable();
        var startRowsAmountInOrderEntity = SQLHelper.getRowAmountFromOrderEntityTable();


        APIHelper.creditPaymentWhenUsedFailedCard(cardInfo);
